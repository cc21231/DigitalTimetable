...
class FormattingOption(tk.Frame):
	...
	def invalid_input(self, elem: tk.Entry, text: str) -> None:
...
class PreviewCell(tk.Frame):
	def __init__(self, root, category, *args, **kwargs):

		super().__init__(*args, background=kwargs['GRID'][1])
		self.root: ExportAsPDFMenu = root
		self.category = category

		kwargs = {k.lower(): v[0] if type(v) is tuple and len(v) == 1 else v for k, v in kwargs.items()}

		if kwargs['align'] == 'CENTER' and kwargs['valign'] == 'MIDDLE':
			anchor = 'center'
		else:
			anchor = {'TOP': 'n', 'BOTTOM': 's', 'MIDDLE': ''}[kwargs['valign']] + {'LEFT': 'w', 'RIGHT': 'e', 'CENTER': ''}[kwargs['align']]

		formatting = dict(
			# highlightbackground=kwargs['grid'][1],
			# highlightthickness=round(kwargs.pop('grid')[0]),
			font=(*kwargs.pop('font')[0].lower().split('-'), round(kwargs['fontsize']) if kwargs['fontsize'] != 'Auto' else 12),
			anchor=anchor
		)
		...
		kwargs.pop('valign')
		kwargs.pop('align')
		kwargs.pop('bottompadding')

		# self.count = kwargs.pop('count') if 'count' in kwargs else 1

		kwargs.update(formatting)

		self.padding = kwargs.pop('grid')[0] * 2
		self.paddingcontrol = kwargs.pop('paddingcontrol')
		self.orient = kwargs.pop('orient')
		self.texts = kwargs.pop('text')
		self.stack = kwargs.pop('side')

		if self.orient == 'vertical':
			kwargs.update({'wraplength': 1})

		self.bind('<ButtonRelease-1>', lambda v: self.root.format_combobox.set(self.category))


		self.labels = []
		for i, text in zip(self.paddingcontrol, self.texts):
			label = tk.Label(self, text=text, **kwargs)
			label.pack(side=self.stack, expand=True, fill='both', padx=(self.padding * i[0], self.padding * i[2]), pady=(self.padding * i[1], self.padding * i[3]))
			self.labels.append(label)
			label.bind('<ButtonRelease-1>', lambda v: self.root.format_combobox.set(self.category))
...
class ExportAsPDFMenu(tk.Toplevel):
	def __init__(self, root, *args, **kwargs) -> None:
		...
		frame = tk.Frame(self, background='#222', width=200)
		frame.grid(row=1, column=1, columns=1, rows=5, sticky='NSWE', padx=(10, 0), pady=(7, 0))
		frame.rowconfigure(1, weight=1)
		frame.columnconfigure(0, weight=1)

		tk.Label(frame, text='Cell Formatting Preview', **labelconfig, highlightthickness=1, highlightbackground='#3B434C').grid(row=0, column=0, sticky='NSWE', pady=(0, 1))

		preview_frame = tk.Frame(frame, background=self.formatting['first_row']['GRID'][1], padx=self.formatting['first_row']['GRID'][0] * 2, pady=self.formatting['first_row']['GRID'][0]*2)
		preview_frame.grid(row=1, column=0, sticky='NSWE', padx=(0, 0), pady=(0, 0))
		preview_frame.rowconfigure(0, weight=1)
		preview_frame.rowconfigure((1, 2, 3), weight=5)

		preview_frame.columnconfigure(0, weight=1)
		preview_frame.columnconfigure(1, weight=5)

		top_header_preview_1 = PreviewCell(self, 'Weekday Headings', preview_frame, text=[''], paddingcontrol=[(0, 0, 1, 1)], side='left', **self.formatting['first_row'])
		top_header_preview_1.grid(row=0, column=0, stick='nswe')

		top_header_preview_2 = PreviewCell(self, 'Weekday Headings', preview_frame, text=['Day'], paddingcontrol=[(0, 0, 0, 1)], side='left', **self.formatting['first_row'])
		top_header_preview_2.grid(row=0, column=1, stick='nswe')

		side_header_preview = PreviewCell(self, 'Session Headings', preview_frame, text=['Session'], paddingcontrol=[(0, 0, 1, 0)], side='left', **self.formatting['first_column'])
		side_header_preview.grid(row=1, column=0, rows=2, stick='nswe')

		session_name_preview = PreviewCell(self, 'Class Names', preview_frame, text=['Class'], paddingcontrol=[(0, 0, 0, 1)], side='left', **self.formatting['sessionname'])
		session_name_preview.grid(row=1, column=1, stick='nswe')

		body_preview = PreviewCell(self, 'Table Body', preview_frame, text=['Room', 'Teacher'], paddingcontrol=[(0, 0, 0, 1), (0, 0, 0, 0)], side='top', **self.formatting['body'])
		body_preview.grid(row=2, column=1, stick='nswe')

		# break_preview_1 = PreviewCell(self, 'Session Breaks', preview_frame, text=[''], paddingcontrol=[(0, 0, 0, 0)], side='left', **self.formatting['break'])
		# break_preview_1.grid(row=3, column=0, stick='nswe')

		break_preview = PreviewCell(self, 'Session Breaks', preview_frame, text=['Break'], paddingcontrol=[(0, 1, 0, 0)], side='left', **self.formatting['break'])
		break_preview.grid(row=3, column=0, columns=2, stick='nswe')

		self.preview_elems = {'Weekday Headings': [top_header_preview_1, top_header_preview_2], 'Session Headings': [side_header_preview], 'Class Names': [session_name_preview], 'Table Body': [body_preview], 'Session Breaks': [break_preview]}

		self.format_combobox = CustomComboBox(frame, state='readonly', style='TCombobox', values=['Weekday Headings', 'Session Headings', 'Class Names', 'Table Body', 'Session Breaks'], width=3)
		self.format_combobox.grid(row=2, column=0, padx=0, pady=(1, 0), sticky='nswe')
		self.format_combobox.bind('<<ComboboxSelected>>', lambda v: self.select_formatting_mode())
		self.format_combobox.set('Weekday Headings')

		format_frame = tk.Frame(frame, background='#3B434C', padx=1, pady=1)

		tk.Label(format_frame, text='Background', **labelconfig, width=50).grid(row=0, column=0, padx=(0, 1), pady=(0, 1), stick='nswe')

		self.bg_entry = ttk.Entry(format_frame, style='stipple.TEntry', width=7)
		self.bg_entry.configure(validatecommand=lambda: self.validate_hex(self.bg_entry), validate='focusout', invalidcommand=lambda: self.invalid_input(self.bg_entry, 'TODO'))
		self.bg_entry.grid(row=0, column=1, padx=(0, 0), pady=(0, 1), stick='nswe')
		self.bg_entry.insert(0, '#D3D3D3')

		## todo: add colour picker button

		tk.Label(format_frame, text='Border', **labelconfig, width=70).grid(row=1, column=0, columns=3, padx=(0, 1), pady=(0, 1), stick='nswe')

		self.borderwidth_entry = ttk.Entry(format_frame, style='stipple.TEntry', width=2)
		self.borderwidth_entry.configure(validatecommand=lambda: self.validate_num(self.borderwidth_entry), validate='focusout', invalidcommand=lambda: self.invalid_input(self.borderwidth_entry, 'TODO'))
		self.borderwidth_entry.grid(row=2, column=0, padx=(0, 1), pady=(0, 1), stick='nswe')
		self.borderwidth_entry.insert(0, '1')

		tk.Label(format_frame, text='px', **labelconfig, width=10).grid(row=2, column=1, padx=(0, 1), pady=(0, 1), stick='nswe')

		self.bordercolour_entry = ttk.Entry(format_frame, style='stipple.TEntry', width=7)
		self.bordercolour_entry.configure(validatecommand=lambda: self.validate_hex(self.bordercolour_entry), validate='focusout', invalidcommand=lambda: self.invalid_input(self.bordercolour_entry, 'TODO'))
		self.bordercolour_entry.grid(row=2, column=2, padx=(0, 0), pady=(0, 1), stick='nswe')
		self.bordercolour_entry.insert(0, '#000000')

		tk.Label(format_frame, text='Font', **labelconfig, width=50).grid(row=3, column=0, columns=3, padx=(0, 1), pady=(5, 1), stick='nswe')

		self.font_entry = CustomComboBox(format_frame, state='readonly', style='TCombobox', width=15)
		# self.fg_entry.configure(validatecommand=lambda: self.validate_hex(self.fg_entry), validate='focusout', invalidcommand=lambda: self.invalid_input(self.fg_entry, 'TODO'))
		self.font_entry.grid(row=4, column=0, columns=3, padx=(0, 0), pady=(0, 1), stick='nswe')
		self.font_entry.insert(0, 'Calibri Bold')

		tk.Label(format_frame, text='Size', **labelconfig, width=20).grid(row=3, column=0, columns=3, padx=(0, 1), pady=(0, 1), stick='nswe')


		tk.Label(format_frame, text='Foreground', **labelconfig, width=50).grid(row=3, column=0, padx=(0, 1), pady=(0, 1), stick='nswe')

		self.fg_entry = ttk.Entry(format_frame, style='stipple.TEntry', width=7)
		self.fg_entry.configure(validatecommand=lambda: self.validate_hex(self.fg_entry), validate='focusout', invalidcommand=lambda: self.invalid_input(self.fg_entry, 'TODO'))
		self.fg_entry.grid(row=1, column=1, padx=(0, 0), pady=(0, 1), stick='nswe')
		self.fg_entry.insert(0, '#000000')
		...
		frame.grid(row=1, column=2, columns=1, rows=5, sticky='NSWE', padx=(10, 0), pady=(7, 0))
		frame.rowconfigure(1, weight=1)
	...
	def convert(self):
		...
		for n, i in enumerate(self.timetable_data['sessions']):
			if i[1]:
				# if self.formatting['first_column']['ORIENT'][0].lower() == 'vertical':
				...
				break_count += 1
				break_idxs.append([(1, len(data)), (-3, len(data))])
				line_data = [i[0]]# + [''] * 7
		...
		for key, row in [('first_row', 0), ('first_column', 1), ('body', 1), ('sessionname', 1), ('break', 1)]:
			...
			if f'{height}.{font}' not in font_sizes:
				...
				# height = (face.ascent - face.descent) / 1000 * (font_size + 0.5)
				while (face.ascent - face.descent) / 1000 * (font_size + 0.5) < height / 2:
					font_size += 0.5
				font_sizes.update({f'{height}.{font}': font_size})

		for key, ranges in [('body', [((1, 1), (-3, -1))]), ('first_row', [((0, 0), (-1, 0))]), ('first_column', [((0, 1), (0, -1))]), ('sessionname', sessionname_idxs), ('break', break_idxs)]:
			for pos in ranges:
						...
						case 'ORIENT':
							if v.lower() == 'vertical':
								...
								for y in data[pos[0][1]:pos[1][1] + (1 if pos[1][1] >= 0 else -1)]:
									...
									line[vertical_slice] = [VerticalText(x, self.formatting[key]['BOTTOMPADDING'][0]) for x in line[vertical_slice]]
									d.append(y)
								# d = [[VerticalText(x, self.formatting[key]['BOTTOMPADDING'][0]) for x in y[pos[0][0]:pos[1][0]]] for y in ]
								data[pos[0][1]:pos[1][1]] = d
						case 'BOTTOMPADDING':
		...
		# print('\n')
		# print(self.tablestyle)
		# print(self.tablestyle[8:30])
